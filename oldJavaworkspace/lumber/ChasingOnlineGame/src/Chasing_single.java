...
public class Chasing_single extends JFrame {
	private static final long serialVersionUID = 1L;
	int defalutdx1 = 7, defalutdy1 = 7, defalutdx2 = 4, defalutdy2 = 4, dx1 = defalutdx1, dy1 = 0,
	...
	JLabel background = new JLabel(new ImageIcon(
			Chasing_single.class.getResource("/images/bgEmpty.jpg")),
			SwingConstants.CENTER);
	playSound sound = new playSound();
	Random randomNumbers = new Random();
	int random = 3;

	public Chasing_single() {
		drawGUI();
	}
	...
	public void drawGUI() {
		...
		Ball1.setIcon(new ImageIcon(Chasing_single.class
				.getResource("/images/square1.png")));
		Ball1.setBounds(6, 6, 32, 32);
		getContentPane().add(Ball1);

		Ball2.setIcon(new ImageIcon(Chasing_single.class
		...
		lblExplosion.setIcon(new ImageIcon(Chasing_single.class.getResource("/images/explode.gif")));
			...
			public void run() {
				if (paused) {
					...
					if (ballOutX(Ball1))
					{
						sound.playHitSound();
						dx1 = -dx1;
						if(Ball1.getX() > screenWidth)
							Ball1.setBounds(screenWidth, Ball1.getY(), Ball1.getWidth(), Ball1.getHeight());
						if(Ball1.getX() < 0)
							Ball1.setBounds(0, Ball1.getY(), Ball1.getWidth(), Ball1.getHeight());
					}
					if (ballOutY(Ball1))
					{
						sound.playHitSound();
						...
						if(Ball1.getY() > screenHeight)
							Ball1.setBounds(Ball1.getX(), 0, Ball1.getWidth(), Ball1.getHeight());
						if(Ball1.getY() < 0)
							Ball1.setBounds(Ball1.getX(), 0, Ball1.getWidth(), Ball1.getHeight());
					}
					...
					//System.out.println("Ball1("+Ball1.getX()+", "+Ball1.getY()+"),Ball2("+Ball2.getX()+", "+Ball2.getY()+")");
	...
	public void random()
	{
		...
		TimerTask ttt = new TimerTask() {
			...
			public void run() {
				random = 1+ randomNumbers.nextInt(3);
				//key1 = random;
				if(Math.abs(Ball1.getX()-Ball2.getX()) > Math.abs(Ball1.getY()-Ball2.getY()))
				{
